...
**Date:** 2 Sept 2026 · **Revision 6**

> ## This document supersedes
>
> **This brief is the single source of truth for this work.** Where anything
> below contradicts an earlier document, this one wins.
>
> - `claude-code-brief-production-roles-workflow.md` (rev 1–3) — **superseded.**
>   Its bucket scope, its "digitizer sends the mockup" flow, and its four-email
>   set are all out of date. Keep it only as background reasoning; build nothing
>   from it.
> - `claude-code-brief-mockup-before-payment.md` (CL43CC) — **retired.** Its
>   digitizer-dispatch half is replaced by Phase 1; its "mockup requested on
>   quotes" half is absorbed into Task 1.3.
> - `panda-crm-roles-audit.md` — still valid as a *survey*, but its claim that
>   role is only a display label was wrong and is corrected in Task 0.1.
...
#### Machine files are in two buckets — fix the leak before flipping

Uploads from **New Order / Edit Order** land in `order-attachments` (public, and
staying public). Uploads from the **order page** land in `production-files` (the
one being locked). So locking `production-files` protects roughly half the stitch
files; the rest stay publicly fetchable and the split keeps growing.

**Decision: do (a) now, schedule (b) separately.**

- **(a) Point the order form at `production-files`.** One line. Stops the split
  growing. **Required to finish Phase 0** — without it the lock is half a lock.
- **(b) Migrate the existing `order-attachments` machine files across.** The
  clean finish. Own task, own verification, after the flip is proven stable.
- (c) Leave it — rejected. It contradicts the whole point of the exercise.

This is not a new decision, it is the same one already made: protect the machine
files. (a) is what makes that true going forward; (b) makes it true historically.
...
**What this is for — read before building the UI.** Post-approval changes are
**absorbed, not charged** (Imran, 2 Sept). So this is not a billing or dispute
mechanism, and it needs no customer-facing "you approved this" confrontation
screen. Its three real jobs are:

1. **Production makes the right patch.** The floor works from the approved
   revision, not from whichever file was uploaded last. This is the one that
   prevents expensive mistakes.
2. **Files are never lost.** A superseded revision stays retrievable — for
   re-orders, remakes, and when a customer changes their mind back.
3. **Rework is visible.** Revision counts per digitizer show where rework is
   coming from. Relevant because the freelancers are paid per job and the
   rework cost is absorbed.

Build it as an internal record, plainly presented. Not an argument.
...
**No shipping dispatch view needed.** Confirmed by Imran: Haseeb receives the
tracking number outside the system, then comes into the portal and updates the
order with it. He is not waiting on a push signal, so there is no gap to close
and `COMPLETED` does not need to be given a new meaning. Do not build it.
