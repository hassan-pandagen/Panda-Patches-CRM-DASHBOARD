...
// Creates a Stripe Checkout Session for a customer to pay an outstanding order balance.
// Verifies the caller is the customer who owns the order (or has cc_email match).
// On successful payment, our stripe-balance-webhook updates orders.amount_paid,
// which triggers the CAPI Purchase event automatically.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Authorization: caller must be the customer or cc_email ──
    ...
    const callerEmail = customerProfile?.email?.toLowerCase();
    ...
    if (["CANCELLED", "REFUNDED"].includes(order.status)) {
      return new Response(
        JSON.stringify({ error: "Cannot pay a cancelled or refunded order" }),
        { headers: { ...getCorsHeaders(req), "Content-Type": "application/json" }, status: 400 }
      );
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Outstanding balance for order ${order.order_number}`,
            },
            unit_amount: Math.round(remaining * 100), // Stripe uses cents
          },
          quantity: 1,
        },
      ],
      ...
      success_url: `https://login.pandapatches.com/customer/order/${order.order_number}?paid=1`,
      cancel_url: `https://login.pandapatches.com/customer/order/${order.order_number}?cancelled=1`,
