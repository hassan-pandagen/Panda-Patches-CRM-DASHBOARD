...
ALTER TABLE public.orders
  ...
  CHECK (country IS NULL OR country IN ('USA', 'AUSTRALIA', 'CANADA', 'NEW ZEALAND', 'UK', 'ICELAND'));
...
COMMENT ON COLUMN public.orders.country IS
  'Shipping country (sales agent picks from a fixed dropdown). Locked to USA / AUSTRALIA / CANADA / NEW ZEALAND / UK / ICELAND. Add new countries by extending the CHECK constraint and the COUNTRY_OPTIONS constant in src/constants/options.ts.';
