...
| *(new hire, starts today)* | TBC | `PRODUCTION_SUPERVISOR` | Create with the supervisor set — do **not** start from the Admin preset |
...
stay **public and unchanged**. They are already emailed to customers, links in
existing emails keep working, and no migration is needed.
...
| Files | Bucket | Access |
|---|---|---|
| Mockups, customer references | `order-attachments` (existing, public) | Unchanged. Do not migrate, do not break existing URLs |
| **Production / machine files** (`production_file_urls`) | **New private bucket `production-files`** | Signed URLs only, 15 min |

Read access to `production-files`:

- `ADMIN`, `PRODUCTION_SUPERVISOR` — always
- `SALES_AGENT` — always
- `DIGITIZER` — **only while the work item is inside their active window**
  (Task 1.2). Access ends when the order moves to production, and returns only
  if a supervisor reassigns it
- Signed out — never

Re-orders with changes: the supervisor reassigns the item to a digitizer, which
reopens access by the normal window rule. No separate mechanism.
...
**Open — see questions:** an earlier instruction asked for a "patches are ready
to ship" internal email on production complete. That is not one of the two
above. Confirm whether it survives.
